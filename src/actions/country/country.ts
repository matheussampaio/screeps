...
import { CityRunner, RoomStopIfConflict, CitySpawner, ICityContext, CityDefense, CityEmergency, CityPlanner, CityLinks, CityMinerals } from '../city'
    ...
    for (const roomName in Game.rooms) {
      ...
      if (process == null) {
        ...
        const PID: number = this.fork({
          memory,
          actions: [[RoomStopIfConflict.name], [CityEmergency.name], [CityRunner.name], [CityDefense.name], [CitySpawner.name], [CityPlanner.name], [CityBuilder.name], [CityLinks.name], [CityMinerals.name]],
          name: `City-${roomName}`
        })
