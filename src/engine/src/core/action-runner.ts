...
export class ActionRunner {
    private static setDefaultAction(agent: Creep | Room, defaults: (string)[][]): void {
    ...
    private static breakResult(actionsResult: (ActionReturn | string)[]): object {
        ...
        if (_.isArray(actionsResult)) {
            parserResult = {
                result: actionsResult[0] as ActionReturn,
    ...
    private static runGroup(agent: Creep | Room, group: string[]): void {
    ...
    private static runGroups(agent: Creep | Room): void {
