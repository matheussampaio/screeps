...
function create({ work = 0, carry = 0, move = 0 }, role, name) {
    const times = Math.floor(Game.rooms.W17N21.energyCapacityAvailable / ( work * 100 + carry * 50 + move * 50));
    const creep = [];
    ...
    if (Game.spawns.Spawn1.createCreep(creep, name, { role }) === OK) {
        console.log(`Created ${role}:`, creep);
    }
}
